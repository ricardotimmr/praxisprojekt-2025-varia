...
const FeatureSliderPreview = ({ config }) => {
  const [currentSlideIndex, setCurrentSlideIndex] = useState(0)
  const [isTransitioning, setIsTransitioning] = useState(false) // To prevent rapid clicks

  // Reset slide index if images array changes or becomes empty
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      >
        <img
          key={currentSlideIndex} // This forces re-render and allows transition to apply
      ...
      {/* Pagination Dots and Autoplay Controls */}
      {config.images.length > 1 && (
        <div
          style={{
            display: 'flex',
        ...
        >
          {config.images.map((_, index) => (
            <button
              ...
              style={{
                ...
                transition: 'all 0.3s',
                backgroundColor:
                  index === currentSlideIndex ? config.accentColor : '#9ca3af', // gray-400
                transform:
                  index === currentSlideIndex ? 'scale(1.25)' : 'scale(1)',
                border: 'none',
                cursor: 'pointer',
                margin: '0 0.25rem', // Equivalent to space-x-2
              }}
          ...
          <button
            ...
            aria-label={
              config.autoplayEnabled ? 'Autoplay pausieren' : 'Autoplay starten'
            }
          >
